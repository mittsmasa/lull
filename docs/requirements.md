...
**Lull** は、発表会などの小規模コンサート・リサイタルを運営するためのWebアプリケーション。
...
2. リンクを受け取った人が `/join/[token]` にアクセス（認証不要）
3. イベント名・開催日時・会場と、表示名の入力欄（主催者が設定した名前をプリフィル）を表示する。「あとで変更できます」のただし書きを添える
4. 「Google アカウントで参加する」ボタンをクリック → Google OAuth でログイン（アカウントが存在しなければ `users` / `accounts` レコードを自動作成）
5. OAuth 完了後、`event_members` に出演者として登録される。`displayName` は出演者がフォームに入力した値を使用する。トークンは使用済み（ステータス: `accepted`）となり、イベント詳細画面にリダイレクト

> **ポイント**: `/join/[token]` ページは認証なしで閲覧可能。参加の意思表示と Google ログインを 1 つのボタンに統合することで、ログインページへのリダイレクトを挟まず、自然な UX を実現する。OAuth コールバックで `/join/[token]` に戻り、サーバー側で参加処理を完了する

#### エッジケース

- **既にログイン済みの場合**: 「Google アカウントで参加する」の代わりに「参加する」ボタンを表示し、クリックで即座に `event_members` に登録する
...
- [x] ログイン後リダイレクト（returnTo）— 未認証アクセス時に元のページへ戻す

### 3.2 イベント管理

- [x] イベント作成（名前、日時、会場、座席数）
- [x] イベント一覧（自分が関わるイベントのみ表示）
- [x] イベント詳細・編集
- [x] イベントステータス管理（draft → published → ongoing → finished）
- [x] イベントの削除
...
- `draft` および `finished` ステータスで削除可能
- `published` / `ongoing` のイベントは削除不可（ゲストに招待リンクが渡っている、または当日進行中のため）
- `published` を削除したい場合は `published → draft` に差し戻してから削除する
- **削除時の注意**: 削除すると、既存の招待・プログラム・メンバー情報はすべてカスケード削除される。削除前に確認ダイアログを表示する。`finished` イベントの場合はチェックイン記録も削除される旨を明示する

### 3.3 メンバー管理

- [x] イベントへの出演者追加（表示名付き招待リンクを発行 → クリップボードコピー or SNS送信）
- [x] 出演者の表示名（displayName）設定・変更
  - [x] 主催者が招待時に表示名を設定（1〜50 文字）
  - [x] 出演者自身が表示名を変更可能（1〜50 文字）
  - [x] 表示名の変更は `draft` / `published` / `ongoing` で可能。`finished` では変更不可
- [x] 出演者の削除（主催者のみ）
  - [x] プログラムに紐づいている出演者は削除不可（先にプログラムから外す必要がある）
  - [x] 削除すると当該出演者の招待リンク（ゲスト向け）はそのまま維持される（招待者名は `inviterDisplayName`（スナップショット）に「（削除済み）」を付けて表示。例:「山田太郎（削除済み）」）
- [x] 出演者の招待リンクの無効化（主催者のみ）
...
- **期限切れ（イベントが `finished` になった後）**: `accepted` の場合はイベント情報・回答内容・受付状況を表示（フォーム・QR非表示）。それ以外は「この招待リンクは期限切れです」エラー画面を表示
...
- [x] QRコードで来場受付
  - [x] QRコードの内容: 招待回答ページの URL（`/i/[token]`）をエンコード
  - [x] チェックイン画面（`/events/[eventId]/checkin`）上でカメラを起動し、ゲストが提示するQRコードを読み取る → URLからトークンを抽出し、該当イベントの招待であることを検証 → ゲストの情報を表示 → 来場確認ボタンを押す
  - [x] 本人・各同伴者の個別来場記録
- [x] リアルタイムの来場者数カウント
- [x] 名前検索によるチェックイン（QRスキャン不可時の代替手段）
- [x] 来場者一覧（Collapsible で展開可能。各行タップでチェックイン操作へ遷移）
- [x] ゲスト向けチェックインステータス表示（招待回答ページで受付状況を確認可能。`ongoing` / `finished` + `accepted` 時に表示）
...
- [ ] ゲストが招待回答ページ（`/i/[token]`）でプログラム一覧を閲覧できる
  - [ ] 出欠回答済み（`accepted`）のゲストに対してプログラム一覧を表示
  - [ ] 表示項目: 順番、種別、タイトル、作曲者、出演者名、所要時間
  - [ ] 読み取り専用（編集操作なし）
  - [ ] イベントステータスが `published` / `ongoing` / `finished` の場合に表示（`draft` では非表示）
...
>
> *○\*\*: ゲストは招待回答ページ（`/i/[token]`）で閲覧のみ可能。`accepted` かつイベントが `published` 以降の場合に限る*
>
> *凡例: ○ = 可能、空白 = 不可（権限なし）*
...
| 期限切れトークン（イベントが `finished`、`accepted` 以外） | 「この招待リンクは期限切れです」 |
...
#### ログイン後リダイレクト（returnTo）

- 認証が必要なページ（`/join/[token]` 等）に未認証でアクセスした場合、ログイン完了後に元のページへ戻す
- `requireSession()` でリダイレクト時に `?returnTo={現在のパス}` をクエリパラメータとしてトップページに付与する
- トップページではクエリパラメータ `returnTo` を読み取り、`signIn.social()` の `callbackURL` に渡す
- ログイン成功後、`returnTo` が指定されていればそのパスへ、未指定なら `/dashboard` へリダイレクトする
- `returnTo` には相対パス（同一オリジン）のみ許可する（オープンリダイレクト脆弱性の防止）
...
1. ~~**メンバー削除時に出演者招待一覧の表示を更新する**: メンバー一覧からメンバーを削除した際、出演者招待一覧に表示されている対応する招待（`accepted` 状態）の表示を消す（または適切に更新する）~~ ✅
2. ~~**出演者招待一覧からの招待リンクコピー**: 招待リンクの発行時だけでなく、出演者招待一覧からもリンクをコピーできるようにする。ただし `accepted`（承認済み）および `invalidated`（無効化済み）の招待ではコピーボタンを表示しない（`pending` のみコピー可能）~~ ✅
